...
const normalizarSlots = (slots: SlotPorDia[]) => {
  const mapa: Record<
    string,
    Record<string, { mitad1: boolean | null; mitad2: boolean | null } | null>
  > = {};
    ...
    for (let h = 8; h < 20; h++) {
      ...
      if (slot1 || slot2) {
        mapa[horaInicio][dia.diaSemana] = {
          mitad1: slot1 ? slot1.disponible : null,
          mitad2: slot2 ? slot2.disponible : null,
        };
      }
    }
  }
...
export default function HorarioDisponibilidad({ slots }: Props) {
  ...
  console.log("📦 Slots originales:", slots);
  console.log("📊 Mapa normalizado:", normalizado);
      ...
      <table className="table-auto text-sm text-white w-full border-collapse border border-neutral-500">
        <thead>
          <tr className="text-center">
            ...
            {DIAS_ORDEN.map((dia) => (
              <th key={dia} className={`p-1 ${dia === diaActualNombre ? "bg-neutral-600/60" : ""}`}>
                {dia}
              </th>
            ))}
          </tr>
          ...
          {horas.map((hora) => (
            <tr key={hora}>
              <td className="p-1 whitespace-nowrap font-semibold text-center">
                {hora} -{" "}
                {`${String(Number(hora.split(":")[0]) + 1).padStart(
                  2,
                  "0"
                )}:00`}
              </td>
